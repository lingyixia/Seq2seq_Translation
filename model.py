...
class Model(object):
    ...
    def seq2seq(self):
        ...
        embed_en_seqs = tf.nn.embedding_lookup(self.en_embedding,
                                               self.en_seqs)  # 词嵌入[1,2,3] --> [[3,...,4],[0.7,...,-3],[6,...,9]],embeding[depth*embedding_size]=[[0.2,...,6],[3,...,4],[0.7,...,-3],[6,...,9],[8,...,-0.7]]，此时的输入节点个数为embedding_size
        ...
        with tf.name_scope("sorfmax_weights"):
            if self.config.share_emb_and_softmax:
                ...
                self.softmax_weight = tf.get_variable("weight", [self.config.hidden_dim,
                                                                 self.config.zh_vocab_size + 1])  # +1是因为对未知的可能输出
        ...
        with tf.name_scope("loss"):
            ...
            loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=tf.reshape(self.zh_seqs_label, [-1]),
                                                                  logits=logits)
    ...
    def train(self, batch_train_g, model_path):
        with self.session as sess:  # (64,50),(64,),(64,50),(64),(64,50)(向右偏移一位)
